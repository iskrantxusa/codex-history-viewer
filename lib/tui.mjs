...
import { copyTextToClipboard, resumeCommand } from "./resume.mjs";
...
export class HistoryTui {
  ...
  async copyResumeCommand() {
    const command = resumeCommand(this.current()?.id);
    const token = ++this.statusToken;
    if (!command) {
      this.statusMessage = " Cannot resume: session id is missing";
      this.render();
      return;
    }
    this.statusMessage = ` Copying: ${command}`;
    this.render();
    try {
      await this.copyText(command);
      if (token !== this.statusToken) return;
      this.statusMessage = ` Copied: ${command}`;
    } catch {
      if (token !== this.statusToken) return;
      this.statusMessage = ` Copy failed: ${command}`;
    }
    this.render();
  }
  ...
  render() {
    ...
    const status = this.searching
      ? ` Search: ${this.query}▌`
      : this.statusMessage || `${matchStatus}  / search  n/N match  r resume  t tools:${this.showTools ? "on" : "off"}  ? help  q quit`;
