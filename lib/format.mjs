...
export function styledSyntaxLine(
  text,
  language = "",
  query = "",
  colors = true,
  ...
  const spans = baseStyle ? [span(0, text.length, baseStyle)] : [];
  spans.push(
    ...tokenSpans(text, language).map((item) => ({
      ...item,
      style: `${baseStyle}${item.style}`,
    })),
  );
  return renderStyledLine({ text, start: 0 }, spans, matches, activeStart, colors);
}
